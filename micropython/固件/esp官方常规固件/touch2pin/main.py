...
def touchOncePin(pNumber):
    touchPin(pNumber)
    time.sleep_ms(50)  #按下时间为50毫秒
    unTouchPin(pNumber)
    time.sleep_ms(50)  #抬起后再等50毫秒
...
def delayt():
    dtmp = randint(40,50)      #取随机数50-100之间
...
def main():
    ...
    while True:  #无限循环
        ...
        if isrun:
            touch1()                     #单数点击一次
            delayRandom(2000,2200)       #随机等待
            touch2()                     #双数点击一次
            delayRandom(250,300)         #一次循环完成，等300-350毫秒进行下次循环      
        else:
            time.sleep_ms(100)
